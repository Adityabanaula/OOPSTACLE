...
public class NetworkCharacterController : NetworkBehaviour
{
	...
	private NetworkCamera networkCam;
	...
	void  Start (){
		...
		cam = GameObject.FindGameObjectWithTag("NetworkCamera");
		networkCam = cam.GetComponent<NetworkCamera>();
	...
	private void Update()
	{
		...
		if(!IsOwner) return;
		networkCam.GetPlayer(transform);
