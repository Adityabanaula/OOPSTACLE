...
public class MovableObs : MonoBehaviour
{
	...
	public float retreatingSpeed = 3f;
    ...
    void Update()
    {
		if (horizontal)
		{
			if (isForward)
			{
				...
				{
					transform.position += Vector3.right * Time.deltaTime * retreatingSpeed;
				}
				else
					isForward = false;
			}
		...
		else
		{
			if (isForward)
			{
				...
				{
					transform.position += Vector3.forward * Time.deltaTime * retreatingSpeed;
				}
				else
					isForward = false;
			}
